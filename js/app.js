...
$(document).ready(function () {
    ...
    $('.intro-slider').on('init', function (event, slick) {
        $('.slick-active .intro-item__content').removeClass('hidden__line');
        $('.slick-active .intro-item__suptitle').removeClass('hidden');
        $('.slick-active .intro-item__title').removeClass('hidden');
        $('.slick-active a').removeClass('hidden__bottom');

    });
    $('.intro-slider').on('beforeChange', function (event, slick, currentSlide, nextSlide) {
        $('.slick-active .intro-item__content').addClass('hidden__line');
        $('.slick-active .intro-item__suptitle').addClass('hidden');
        $('.slick-active .intro-item__title').addClass('hidden');
        $('.slick-active a').addClass('hidden__bottom');

    });

    $('.intro-slider').on('afterChange', function (event, slick, currentSlide) {
        $('.slick-active .intro-item__content').removeClass('hidden__line');
        $('.slick-active .intro-item__suptitle').removeClass('hidden');
        $('.slick-active .intro-item__title').removeClass('hidden');
        $('.slick-active a').removeClass('hidden__bottom');
...
// ДЛЯ СВЯЗКИ ДВУХ СЛАЙДЕРОВ НЕОБХОДИМО ОБЬЯВИТЬ ИХ В ПЕРЕМЕННУЮ let sliderName = new Swiper('.class-slider', {
...
let myImageSlider = new Swiper('.image-slider', {
    ...
    // Включение/отключение перетаскивания на ПК (Свайпить) true или false
    simulateTouch: true,
    ...
    //     // будет срабатывать прокрутка мышью. Если их несколько то будут крутится все
    //     // Чтобы этого не было не указываем eventsTarget: '.----'
    ...
    // Можно указывать значение допустим 1.5  2.5  2.2
    slidesPerView: 1,
    // slidesPerView: 'auto',

    // Если слайдов меньше чем указано в slidesPerView то слайдер отключится автоматически 
    // А слайдов будет достаточно то он активируется автоматически
    watchOverflow: true,
    ...
    // Бесконечный слайдер. Не работает с мультирядностью
    loop: false,

    // Количество дублирующих слайдов
    loopedSlides: 0,

    // Свободный режим, нет задержки анимации
    freeMode: true,
    ...
    // Миниатюры =========================================
    // thumbs: {
    //     // Слайдер с миниатюрами и его настройки
    //     // Для большого слайдера slidesPerView: 1, иначе работает не коректно
    //     swiper: {
...
$(document).ready(function () {
    ...
    $(document).ready(function (event) {
        $('.tabs-block__title').click(function (event) {
            if ($('.tabs-block').hasClass('one')) {
                $('.tabs-block__title').not($(this)).removeClass('active');
                $('.tabs-block__text').not($(this).next()).slideUp(300);
            }
    ...
    $(window).bind('scroll', function () {
        ...
        if (windowTop > reviewsTop) {
            ...
            $('.footer__map').html('<iframe class="map-iframe" src="https://www.google.com/maps/embed?pb=!1m10!1m8!1m3!1d1921.6503923782643!2d36.231968986502274!3d49.990818745415154!3m2!1i1024!2i768!4f13.1!5e0!3m2!1sru!2sua!4v1594928510622!5m2!1sru!2sua" frameborder="0" style="border:0;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>');
...
const nextYear = new Date(`January 01 ${currentYear + 2} 00:00:00`);
...
let liArr = document.querySelectorAll('.offers__hide');
const offersBtn = document.querySelector('.offers__link');

for (let i = 0; i < liArr.length; i++) {
    offersBtn.addEventListener('click', function (event) {
        event.preventDefault();
        liArr[i].classList.toggle('offers__hide');
    });

}

// ==============================================
let imagesSourceAll = document.querySelectorAll('source');
// console.log(imagesSourceAll);
for (let i = 0; i < imagesSourceAll.length; i++) {
    imagesSourceAll[i].setAttribute('loading', 'lazy');
}

let imagesAll = document.querySelectorAll('img');
// console.log(imagesAll);
for (let i = 0; i < imagesAll.length; i++) {
    imagesAll[i].setAttribute('loading', 'lazy');
}
